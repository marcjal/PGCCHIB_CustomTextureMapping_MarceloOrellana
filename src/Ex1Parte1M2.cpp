 ...
 */
 ...
 // No vertex shader usamos "attribute" (não "layout") e no fragment shader usamos "gl_FragColor".
 ...
 int main()
 {
	 // Inicializa a GLFW (sem definir hints de versão para usar o contexto padrão OpenGL 2.1)
	 if (!glfwInit()) {
	 ...
	 // Inicializa a paleta de cores (valores originais, depois normalizados)
	 ...
	 // Cria um triângulo padrão utilizando apenas um VBO (sem VAOs)
	 GLuint triangleVBO = createTriangleVBO(-0.5f, -0.5f, 0.5f, -0.5f, 0.0f, 0.5f);
	 
	 // Cria o primeiro triângulo (armazenado na lista) com posição, dimensões e cor
	 Triangle tri;
	 ...
	 // Localiza a variável uniform para a cor no shader
	 ...
	 while (!glfwWindowShouldClose(window))
	 {
		 ...
		 // Para cada triângulo armazenado, atualiza a matriz de modelo e a cor, e desenha usando o VBO
		 for (size_t i = 0; i < triangles.size(); i++)
		 {
			 ...
			 // Como não usamos VAOs, configura o VBO e os atributos a cada vez antes de desenhar
 ...
 // Função que compila os shaders e cria o programa (linka os shaders)
 // Usamos GLSL 120 para compatibilidade com OpenGL 2.1
 int setupShader()
 {
	 ...
	 // Cria o programa de shader e anexa os shaders
	 GLuint shaderProgram = glCreateProgram();
	 glAttachShader(shaderProgram, vertexShader);
	 glAttachShader(shaderProgram, fragmentShader);
	 
	 // Liga manualmente o atributo "position" à localização 0 (opcional, já que configuramos o ponteiro diretamente)
 ...
 // Função que cria um VBO com a geometria (vértices) de um triângulo
 // Nesta versão, NÃO usamos VAOs para máxima compatibilidade com OpenGL 2.1.
 GLuint createTriangleVBO(float x0, float y0, float x1, float y1, float x2, float y2)
 {
	 GLfloat vertices[] = {
		 x0, y0, 0.0f,
 ...
 // Callback de mouse – cria um novo triângulo na posição do clique
 void mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
 {
